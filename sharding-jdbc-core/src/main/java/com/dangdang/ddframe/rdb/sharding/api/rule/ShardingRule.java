...
public final class ShardingRule
{
	...
	public ShardingRule(final DataSourceRule dataSourceRule, final Collection<TableRule> tableRules,
			final Collection<BindingTableRule> bindingTableRules,
			...
			final KeyGenerator keyGenerator)
	{
		...
		this.bindingTableRules = null == bindingTableRules ? Collections.<BindingTableRule> emptyList()
				: bindingTableRules;
			...
			final DataSourceRule defaultDataSourceRule)
	{
		...
		defaultDataSourceMap.put(defaultDataSourceRule.getDefaultDataSourceName(),
	...
	{
		if (databaseShardingStrategy.getShardingColumns().contains(column.getName())
		...
		for (TableRule each : tableRules)
		{
			...
			if (null != each.getDatabaseShardingStrategy()
					&& each.getDatabaseShardingStrategy().getShardingColumns().contains(column.getName()))
			{
				return true;
			}
			if (null != each.getTableShardingStrategy()
					&& each.getTableShardingStrategy().getShardingColumns().contains(column.getName()))
			{
				return true;
			}
		}
