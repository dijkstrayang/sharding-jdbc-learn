...
public final class SimpleRoutingEngine implements RoutingEngine
{
	...
	private Collection<String> routeDataSources(final TableRule tableRule)
	{
		...
		logBeforeRoute("database", logicTableName, tableRule.getActualDatasourceNames(), strategy.getShardingColumns(),
				shardingValues);
		//分库规则没有动态分片，仅有静态分片。
		Collection<String> result = strategy.doStaticSharding(sqlStatement.getType(),
	...
	private Collection<String> routeTables(final TableRule tableRule, final Collection<String> routedDataSources)
	{
		...
		List<ShardingValue<?>> shardingValues = HintManagerHolder.isUseShardingHint()
				...
				: getShardingValues(strategy.getShardingColumns());
		logBeforeRoute("table", logicTableName, tableRule.getActualTables(), strategy.getShardingColumns(),
				shardingValues);
        // 判断是静态分片，还是动态分片。
	...
	private List<ShardingValue<?>> getDatabaseShardingValuesFromHint(final Collection<String> shardingColumns)
	{
		...
		{
			Optional<ShardingValue<?>> shardingValue = HintManagerHolder
					.getDatabaseShardingValue(new ShardingKey(logicTableName, each));
	...
	private List<ShardingValue<?>> getTableShardingValuesFromHint(final Collection<String> shardingColumns)
	{
		...
		{
			Optional<ShardingValue<?>> shardingValue = HintManagerHolder
					.getTableShardingValue(new ShardingKey(logicTableName, each));
			if (shardingValue.isPresent())
			{
				result.add(shardingValue.get());
			}
	...
	private void logBeforeRoute(final String type, final String logicTable, final Collection<?> targets,
			final Collection<String> shardingColumns, final List<ShardingValue<?>> shardingValues)
	{
		log.debug("Before {} sharding {} routes db names: {} sharding columns: {} sharding values: {}", type,
				logicTable, targets, shardingColumns, shardingValues);
	}
	...
	private RoutingResult generateRoutingResult(final TableRule tableRule, final Collection<String> routedDataSources,
			final Collection<String> routedTables)
	{
		RoutingResult result = new RoutingResult();
		for (DataNode each : tableRule.getActualDataNodes(routedDataSources, routedTables))
		{
			result.getTableUnits().getTableUnits()
					.add(new TableUnit(each.getDataSourceName(), logicTableName, each.getTableName()));
		}
		return result;
	}
}
